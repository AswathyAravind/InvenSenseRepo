...
import com.gapblue.salesdashboard.db.ForecastHPreparedStatementSetter;
...
@Configuration
public class DatabaseForecastHistoryWriter implements ItemWriter<ForecastHistoryC>,StepExecutionListener{
	private static final Logger LOGGER = LoggerFactory.getLogger(DatabaseForecastHistoryWriter.class);

	 
	 private static final String QUERY_INSERT_FORECASTHISTORY="MERGE INTO XXINV_FORECASTHISTORY a USING (SELECT ? Id , ? RecordName ,? ObjectVersionNumber ,? CpdrfVerSor ,? JobDefinitionPackage ,? RequestId , "+
...
public DataSource batchDatasource;
			...
			public ExitStatus afterStep(StepExecution stepExecution) {
				...
				if(stepExecution.getStatus() == BatchStatus.COMPLETED){
					LOGGER.debug("soapForecastDBJob job completed successfully");
				salesDashboardUtils.setInterfacePoolDateTime("soapForecastHDBJob",batchDatasource);
	...
	@Override
	public void write(List<? extends ForecastHistoryC> items) throws Exception {
		LOGGER.debug("Write to database: ");
		JdbcBatchItemWriter<ForecastHistoryC> databaseItemWriter = new JdbcBatchItemWriter<>();
		databaseItemWriter.setDataSource(secondaryDataSource);
		databaseItemWriter.setSql(QUERY_INSERT_FORECASTHISTORY);
		ItemPreparedStatementSetter<ForecastHistoryC> forecastPreparedStatementSetter = new ForecastHPreparedStatementSetter();
